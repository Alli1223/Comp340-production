...
public class TempShoot : MonoBehaviour
{
    ...
    void Update()
    {
        if (Input.GetMouseButtonDown(1) && tManage.tPlayer.shootingMode && PlayerManager.currentPlayer.GetComponent<TempPlayerVar>().currentAP >= 6)
        {
            PlayerManager.currentPlayer.GetComponent<TempPlayerVar> ().currentAP -= 6;
            ...
            if (Physics.Raycast(testRay, out hit, 500))
            {
                ...
                {
                    Vector3 dir = (hit.transform.position - PlayerManager.currentPlayer.transform.position);
                    GameObject firedBull = Instantiate(bullet, PlayerManager.currentPlayer.transform.position, Quaternion.LookRotation(dir));
