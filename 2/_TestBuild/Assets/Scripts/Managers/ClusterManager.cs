...
public static class ClusterManager 
{
    ...
    private static void FillClusters()
    {
        ...
        foreach (GameObject x in indObj)
        {
			...
			Tile holder = GridPositionDetection.GetClosestGrid(x.transform.position, tManage.tGrid.tileVariables);
...
//        }
    ...
    {           
        foreach (idHolder x in items)
        {
            ...
            {
                int dist = GridPositionDetection.DistCheck(currentID.currentAI.myPosition, x.currentAI.myPosition);
                if (dist < GridPositionDetection.gpd_OuterCircle || dist < GridPositionDetection.gpd_InnerCircle)
                {
        ...
        {  
            ...
            if (removedList.Contains(currIDs[first]))
            {
                ...
                {  
                    ...
                    {
                        int dist = GridPositionDetection.DistCheck(currIDs[first].currentAI.myPosition, currIDs[second].currentAI.myPosition);
                        if (dist < GridPositionDetection.gpd_OuterCircle || dist <= GridPositionDetection.gpd_InnerCircle)
                        {
                            b++;
                            for (int third = 0; third < currIDs.Count; third++)
                            {
                                ...
                                {
                                    int distance = GridPositionDetection.DistCheck(currIDs[second].currentAI.myPosition, currIDs[third].currentAI.myPosition);
                                    if (distance <= GridPositionDetection.gpd_InnerCircle)
                                    {
